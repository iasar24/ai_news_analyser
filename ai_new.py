...
def highlight_abusive_words(text, ai_detected_elements=None):
    """Highlight abusive words in the text, including AI-detected ones"""
    # First highlight predefined keywords
    for word in ABUSIVE_KEYWORDS:
        pattern = re.compile(r'\b' + word + r'\b', re.IGNORECASE)
        text = pattern.sub(f'<span style="color:red;font-weight:bold">{word}</span>', text)
    
    # Then highlight AI-detected elements if any
    if ai_detected_elements:
        for element in ai_detected_elements:
            if element and len(element) > 2:  # Avoid highlighting very short strings
                # Use a more flexible pattern for phrases
                pattern = re.compile(re.escape(element), re.IGNORECASE)
                text = pattern.sub(f'<span style="color:orange;font-weight:bold">{element}</span>', text)
